...
LAMMPS + MACE version of the NVT workflow.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        help=(
            ...
            "<outdir>/<PMC_ID>/<MODEL>/NVT_results. Defaults to the current "
    ...
    p.add_argument(
        ...
        type=float,
        default=1000.0,
        help="NVT production duration in ps (default: 1000 = 1 ns)",
    )
    p.add_argument(
        "--timestep-fs",
        type=float,
    ...
    p.add_argument(
        ...
        help=(
            ...
            "each NVT stage (the 'restart N file.*' command), separate from the "
            "single write_restart at the end of the stage. Default: 100. "
            "Use e.g. 50 for more frequent checkpoints, or a larger value to "
            "write fewer intermediate checkpoint files."
        ),
    )
    p.add_argument(
        "--resume-policy",
...
def main():
    ...
    # Results root: <outdir or cwd>/<PMC_ID>/<model_tag>/NVT_results
