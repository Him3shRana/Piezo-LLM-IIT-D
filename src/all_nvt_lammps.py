...
compound in LAMMPS, driven by ANY MACE-family model registered below (mace /
mace-off23 / polarmace, any size) -- provided that model has a LAMMPS-
compatible .pt export. Three stages run in sequence, one script call:
...
Usage (steps):
    python run_nvt_lammps.py --pmc PMC-023 --model mace-off23 --version medium \
        --temperature 300 --minimize-steps 500 --equil-step 5000 --target-step 50000

Usage (picoseconds instead of steps -- converted internally, everything past
this point still operates in step-counts):
    python run_nvt_lammps.py --pmc PMC-023 --model mace-off23 --version medium \
        --temperature 300 --minimize-steps 500 --equil-ps 10 --target-ps 100
...
3. THE THREE STAGES, IN ORDER, ONE SCRIPT CALL:
     minimization   -> LBFGS-style energy minimization, T=0, no thermostat
     equilibration  -> fix nvt, brings the system up to --temperature
     production     -> fix nvt, the actual sampling run -> traj.extxyz
...
4. WHERE OUTPUT GOES -- everything lands under:
     <--runs-dir>/<PMC-ID>/lammps-nvt/<model>-<version>/<T>K_<N>x<N>x<N>/
       minimization/    equilibration/    production/
   This nesting (engine-ensemble / model-version / temperature+supercell) is
   deliberate: running the same compound at several temperatures produces
   clean SIBLING folders under the same model-version, instead of one long
   unwieldy folder name -- and syncing only LAMMPS-NVT results elsewhere
   (e.g. to a local machine) is then just "grab the lammps-nvt/ subtree."
...
# both use fix nvt but read_restart differs on resume).
# ---------------------------------------------------------------------------
...
COMMON_HEADER = """\
# Auto-generated by run_nvt_lammps.py -- do not hand-edit, re-run the script instead.
...
def run_equilibration_or_production(stage_name, template, stage_dir, pmc_id, model, data_path,
    ...
    if traj_filename:
        # This LAMMPS build lacks the EXTRA-DUMP package, so `dump ... extxyz`
        # isn't available -- dump the universally-supported `custom` style
        # instead (traj_raw.lammpstrj), then convert to the standardized
        # .extxyz format ourselves via ASE. The raw .lammpstrj is deleted
        # afterward so only the final traj.extxyz remains (fewer files).
...
def parse_args():
    p = argparse.ArgumentParser(description="Full minimize -> equilibrate -> produce NVT pipeline, any MACE-family model, LAMMPS engine.")
...
def main():
    ...
    # Unified --timestep is in femtoseconds (same unit across all 4 scripts);
    # LAMMPS's `units metal` natively wants picoseconds, so convert once here
    # and use timestep_ps for every LAMMPS-facing calculation from this point on.
    ...
    condition_dir = os.path.join(
        args.runs_dir, args.pmc, "lammps-nvt", f"{model['family']}-{model['size']}",
        f"{int(args.temperature)}K_{args.supercell_size}x{args.supercell_size}x{args.supercell_size}",
    )
    os.makedirs(condition_dir, exist_ok=True)
    ...
    log(f"[{args.pmc}] engine=lammps model={model['family']}/{model['size']} ({model['lammps_pt']}) ensemble=NVT T={args.temperature}K")
...
if __name__ == "__main__":
    try:
        ...
        raise  # already-clear errors (missing args, GPU check, etc) handled their own messaging
    except Exception as exc:
